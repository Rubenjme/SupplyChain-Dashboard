...
df = pd.read_csv(csv_path) # Leo el CSV
...
kpi_data = { # Diccionario con los datos de los KPIs
    "KPI": [
        "Total Orders",
        "On Time Delivery (%)",
...
kpi_df = pd.DataFrame(kpi_data) # Creo un DataFrame con los KPIs


kpi_csv_path = os.path.join(data_dir, 'supply_chain_kpis.csv') # Ruta al archivo CSV de KPIs (data/supply_chain_kpis.csv)
kpi_df.to_csv(kpi_csv_path, index=False)                       # Guardo el DataFrame en un archivo CSV
